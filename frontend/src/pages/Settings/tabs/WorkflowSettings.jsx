        ...
        <div className="space-y-8">
            ...
            <div className="bg-base-50 p-6 rounded-2xl border border-base-200">
                <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                    ...
                    {t('settings.workflow.approvals_title')}
                </h3>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="form-group">
                        <label className="form-label">{t('settings.workflow.po_limit')}</label>
                    ...
                    <div className="form-group">
                        <label className="form-label">{t('settings.workflow.credit_limit_approval')}</label>
                        <select
                            className="form-input"
                            value={settings.workflow_credit_override || 'manager'}
                            onChange={(e) => handleSettingChange('workflow_credit_override', e.target.value)}
                        >
                            <option value="block">{t('settings.workflow.action_block')}</option>
                            <option value="manager">{t('settings.workflow.action_manager')}</option>
                            <option value="warn">{t('settings.workflow.action_warn')}</option>
                        </select>
                    </div>
                </div>
                ...
                <div className="mt-6 space-y-3">
                    <div className="flex items-center gap-3">
                        ...
                        <label className="cursor-pointer font-medium" htmlFor="workflow_sales_return">
                            {t('settings.workflow.return_approval')}
                        </label>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <label className="cursor-pointer font-medium" htmlFor="workflow_discount_limit">
                            {t('settings.workflow.discount_approval')}
                        </label>
                    </div>
                </div>
            </div>
