...
| **الأولوية** | P1 |
| **الجهد** | 2-4 أسابيع |
| **الحالة** | 🚧 قيد التنفيذ المتقدم (تم تفعيل Alembic + ربط provisioning + ORM phase1/phase2/phase3/phase4/phase6/phase7/phase8/phase9/phase10/phase11/phase12/phase13/phase14/phase15/phase16/phase17/phase18/phase19/phase20/phase21/phase22/phase23/phase24/phase25/phase26/phase27/phase28/phase29/phase30/phase31/phase32/phase33 + drift-fix إضافي لـ `asset_impairments` و`expense_policies` + bulk drift normalization + revision ثامن: `c1f4e8d9b2a6`) |
...
**مخرجات Phase 27 ORM coverage (مطبقة على الشركات الحالية + النظام):**
- إضافة نماذج ORM لوحدة maintenance/manufacturing/marketing: `maintenance_logs`, `manufacturing_equipment`, `manufacturing_operations`, `manufacturing_routes`, `marketing_campaigns`.
- توسيع `MODELED_TABLES` من 156 إلى 161 جدولًا.
- نتيجة التحقق: الجداول الخمسة مستقرة عبر جميع الشركات الحالية (`present=2/2`, `ordered_variants=1`, `set_variants=1`) ولا يوجد missing modeled tables.
...
**مخرجات Phase 30 ORM coverage (مطبقة على الشركات الحالية + النظام):**
- إضافة نماذج ORM (دفعة موسعة 10 جداول) لوحدة payments/pending/performance/pos loyalty: `payment_vouchers`, `payments`, `pending_payables`, `pending_receivables`, `performance_reviews`, `pos_kitchen_orders`, `pos_loyalty_points`, `pos_loyalty_programs`, `pos_loyalty_transactions`, `pos_order_lines`.
- توسيع `MODELED_TABLES` من 171 إلى 181 جدولًا.
- نتيجة التحقق: الجداول العشرة مستقرة عبر جميع الشركات الحالية (`present=2/2`, `ordered_variants=1`, `set_variants=1`) ولا يوجد missing modeled tables.

**مخرجات Phase 31 ORM coverage (مطبقة على الشركات الحالية + النظام):**
- إضافة نماذج ORM (دفعة موسعة 20 جدول) لتقليل عدد ملفات `phase` وعدد مرات الرفع: `pos_order_payments`, `pos_orders`, `pos_payments`, `pos_promotions`, `pos_return_items`, `pos_returns`, `pos_sessions`, `pos_table_orders`, `pos_tables`, `print_templates`, `production_order_operations`, `production_orders`, `project_risks`, `project_timesheets`, `purchase_agreement_lines`, `purchase_agreements`, `receipts`, `recurring_journal_lines`, `recurring_journal_templates`, `report_templates`.
- توسيع `MODELED_TABLES` من 181 إلى 201 جدولًا.
- نتيجة التحقق: الجداول العشرون مستقرة عبر جميع الشركات الحالية (`present=2/2`, `ordered_variants=1`, `set_variants=1`) ولا يوجد missing modeled tables.

**مخرجات Phase 32 ORM coverage (مطبقة على الشركات الحالية + النظام):**
- إضافة نماذج ORM (دفعة موسعة 20 جدول) لوحدة sales/RFQ/security/stock: `request_for_quotations`, `revenue_recognition_schedules`, `rfq_lines`, `rfq_responses`, `roles`, `sales_commissions`, `sales_opportunities`, `sales_order_lines`, `sales_orders`, `sales_quotation_lines`, `sales_quotations`, `sales_return_lines`, `sales_returns`, `sales_targets`, `scheduled_reports`, `security_events`, `shared_reports`, `stock_shipment_items`, `stock_shipments`, `stock_transfer_log`.
- توسيع `MODELED_TABLES` من 201 إلى 221 جدولًا.
- نتيجة التحقق: الجداول العشرون مستقرة عبر جميع الشركات الحالية (`present=2/2`, `ordered_variants=1`, `set_variants=1`) ولا يوجد missing modeled tables.

**مخرجات Phase 33 ORM coverage (مطبقة على الشركات الحالية + النظام):**
- إضافة نماذج ORM (دفعة نهائية 19 جدول أعمال) لوحدة suppliers/tax/training/security/webhooks: `supplier_balances`, `supplier_bank_accounts`, `supplier_contacts`, `supplier_groups`, `supplier_payments`, `supplier_ratings`, `supplier_transactions`, `suppliers`, `task_dependencies`, `tax_calendar`, `training_participants`, `training_programs`, `user_2fa_settings`, `user_sessions`, `webhook_logs`, `webhooks`, `wht_rates`, `wht_transactions`, `work_centers`.
- توسيع `MODELED_TABLES` من 221 إلى 240 جدولًا.
- نتيجة التحقق: الجداول التسعة عشر مستقرة عبر جميع الشركات الحالية (`present=2/2`, `ordered_variants=1`, `set_variants=1`) ولا يوجد missing modeled tables، والمتبقي فقط `alembic_version` كجدول نظامي غير مستهدف في ORM.
