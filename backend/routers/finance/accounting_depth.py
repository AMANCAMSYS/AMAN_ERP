...
def ecl_list(db=Depends(get_db_connection), limit: int = 50):
    rows = db.execute(text("""
        SELECT id, as_of_date, total_exposure, provision_amount, journal_entry_id, created_at
...
def nrv_list(db=Depends(get_db_connection), limit: int = 100):
    rows = db.execute(text("""
...
def cgu_create(body: CGUCreateRequest, db=Depends(get_db_connection)):
    row = db.execute(text("""
...
def cgu_list(db=Depends(get_db_connection)):
    rows = db.execute(text(
...
def ifrs15_create_contract(body: ContractCreateRequest, db=Depends(get_db_connection)):
    try:
...
def ifrs15_get_contract(contract_id: int, db=Depends(get_db_connection)):
    c = db.execute(text(
...
def einvoice_submit(body: EInvoiceSubmitRequest, db=Depends(get_db_connection)):
    try:
...
def einvoice_list(db=Depends(get_db_connection), limit: int = 50,
...
def einvoice_refresh(submission_id: int, db=Depends(get_db_connection)):
    sub = db.execute(text("""
        SELECT id, jurisdiction, document_uuid FROM e_invoice_submissions WHERE id = :id
