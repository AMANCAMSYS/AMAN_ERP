...
- **TASK-036 [M]** ✅ — **IFRS 15 / ASC 606**: جداول `revenue_contracts` + `performance_obligations` + [ifrs15_revenue_service.py](../backend/services/ifrs15_revenue_service.py) (relative-SSP allocation + point-in-time & over-time recognition + optional JE). API: `POST /finance/accounting-depth/ifrs15/contracts` و `POST /ifrs15/recognise`. 5 tests في [test_49_ifrs15_revenue.py](../backend/tests/test_49_ifrs15_revenue.py).
- **TASK-037 [M]** ✅ — **IFRS 9 ECL**: `ecl_rate_matrix` (7 buckets seeded) + `ecl_provisions` + [ecl_service.py](../backend/services/ecl_service.py). API: `POST /finance/accounting-depth/ecl/compute` و `GET /ecl/provisions`.
- **TASK-038 [M]** ✅ — **IAS 2 NRV**: `inventory_nrv_tests` + [nrv_service.py](../backend/services/nrv_service.py). API: `POST /finance/accounting-depth/nrv/run` و `GET /nrv/tests`.
- **TASK-039 [M]** ✅ — **IAS 36 Impairment**: `cash_generating_units` + `impairment_tests` + [impairment_service.py](../backend/services/impairment_service.py). API: `POST /cgu`, `POST /impairment/test`. قيم VIU/FVLCS تُمرَّر من المستخدم (DCF خارج نطاق النواة).
- **TASK-040 [M]** ✅ — **E-Invoicing**: [integrations/einvoicing/](../backend/integrations/einvoicing/) بـ `EInvoiceAdapter` protocol + `EgyptETAAdapter` + `UAEFTAAdapter` (dry-run افتراضياً). API: `POST /finance/accounting-depth/einvoice/submit` + `GET /submissions` + `POST /{id}/refresh`. التحوّل إلى live يحتاج credentials فقط (`ETA_CLIENT_ID`, `UAE_ASP_ENDPOINT`).
...
**ملاحظة نطاق:** محرّك IFRS 15 يدعم relative-SSP + point-in-time + over-time recognition. القرارات المتروكة لكل tenant هي CoA mapping (حسابات الإيراد/الأصل التعاقدي)، معدّلات الخصم لـ VIU في IAS 36، واعتمادات ETA/ASP للـ e-invoicing — كل هذه تُمرَّر كـ parameters أو env vars دون الحاجة لأي schema churn.
