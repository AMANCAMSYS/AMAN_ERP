...
- [x] **[HR-005]** مسيرات الرواتب المطبوعة (Payslip) ✅ (باكند + فرونتإند)
  - [x] قالب مسير راتب (باكند)
  - [x] طباعة PDF (فرونتإند) ✅ — `pages/HR/Payslips.jsx`
  - [x] إرسال بالبريد الإلكتروني (فرونتإند) ✅ — `pages/HR/Payslips.jsx`
  - [x] بوابة موظف لتحميل المسير (فرونتإند) ✅ — `pages/HR/Payslips.jsx`

### 4.2 الحضور والإجازات

- [x] **[HR-006]** رصيد الإجازات المرحّل ✅ (باكند + فرونتإند)
  - [x] ترحيل رصيد نهاية السنة (باكند)
  - [x] حد أقصى للترحيل (باكند)
  - [x] انتهاء صلاحية الرصيد المرحّل (باكند)
  - [x] واجهة ترحيل الإجازات (فرونتإند) ✅ — `pages/HR/LeaveCarryover.jsx`
...
- [x] **[HR-012]** التوظيف والاستقطاب ✅ (باكند + فرونتإند)
  ...
  - [x] قبول/رفض (باكند)
  - [x] واجهة التوظيف (فرونتإند) ✅ — `pages/HR/Recruitment.jsx`
...
- [x] **[MFG-109]** تقارير التصنيع الشاملة ✅
  ...
  - [x] تقرير العمالة المباشرة (Direct Labor Report) ✅ — `pages/Manufacturing/DirectLaborReport.jsx`
...
- [x] **[POS-001]** وضع عدم الاتصال (Offline Mode) ✅ (فرونتإند)
  - [x] Service Worker — `POSOfflineManager.jsx`
  - [x] IndexedDB للتخزين المحلي — `DB_NAME = 'aman_pos_offline'`
  - [x] مزامنة تلقائية عند الاتصال — `syncPendingOrders()`
  - [x] قائمة انتظار للمعاملات — `getPendingOrders()` + `markSynced()`
- [x] **[POS-002]** طباعة فاتورة حرارية ✅ (فرونتإند)
  - [x] تكامل مع طابعات حرارية (ESC/POS) — `ThermalPrintSettings.jsx`
  - [x] قوالب طباعة قابلة للتخصيص — 80mm + 58mm
  - [x] طباعة تلقائية بعد البيع
  - [x] حجم ورق مخصص + دعم شعار الشركة
...
- [x] **[POS-004]** برنامج ولاء (Loyalty) ✅ (باكند)
  ...
  - [x] مكافآت خاصة
- [x] **[POS-005]** شاشة عرض للعميل + فتح/إغلاق درج النقد ✅ (فرونتإند)
  - [x] عرض المنتجات والأسعار والإجمالي — `CustomerDisplay.jsx`
  - [x] أمر فتح الدرج وتكامل مع الأجهزة — `drawerMode: auto/manual/disabled`
...
- [x] **[SALES-005]** طباعة فواتير بتنسيقات متعددة ✅ (فرونتإند)
  - [x] قوالب متعددة (رسمي/مبسط/تجاري) — `InvoicePrintModal.jsx`
  - [x] تخصيص حسب العميل + شعار + ألوان مخصصة
...
- [x] **[CRM-002]** الفرص البيعية (Opportunities) ✅ (باكند + فرونتإند)
  ...
  - [x] تحويل إلى عرض سعر ✅ — `POST /crm/opportunities/{id}/convert-quotation`
- [x] **[CRM-003]** الحملات التسويقية ✅ (باكند + فرونتإند)
  - [x] جدول `marketing_campaigns` ✅
  - [x] استهداف شرائح العملاء + إرسال بريد جماعي ✅ — `routers/crm.py`
  - [x] تتبع النتائج (Open Rate, Click Rate) ✅ — `routers/crm.py`
...
- [x] **[CRM-005]** قاعدة المعرفة ✅ (باكند + فرونتإند)
  - [x] جدول `crm_knowledge_base` ✅
  - [x] أسئلة شائعة (FAQ) + مقالات مساعدة + بحث ✅ — `routers/crm.py` + `pages/CRM/KnowledgeBase.jsx`

### 9.12 إدارة الخدمات (Service Management)

> 📝 **الملفات المُنشأة:**
> - `backend/routers/services.py` — Service Requests CRUD + Document Management
> - `backend/migrations/migrate_services.py` — 4 جداول (service_requests, service_request_costs, documents, document_versions)
> - `frontend/src/services/services.js` — API client
> - `frontend/src/pages/Services/ServicesHome.jsx` — الصفحة الرئيسية
> - `frontend/src/pages/Services/ServiceRequests.jsx` — طلبات الصيانة
> - `frontend/src/pages/Services/DocumentManagement.jsx` — إدارة المستندات

- [x] **[SVC-001]** إدارة طلبات الصيانة ✅ (باكند + فرونتإند)
  - [x] جدول `service_requests` + `service_request_costs`
  - [x] فتح طلب صيانة + ترقيم تلقائي (SRQ-YYYY-XXXX)
  - [x] تعيين فني + تتبع حالة الطلب (pending → assigned → in_progress → on_hold → completed → cancelled)
  - [x] تتبع تكاليف الصيانة (أجور عمالة، قطع غيار، تنقل، أخرى)
  - [x] إحصائيات: إجمالي الطلبات، متوسط الساعات، إجمالي التكاليف، حالات حرجة
  - [x] واجهة كاملة مع فلاتر وتفاصيل موسعة وإدارة التكاليف
- [x] **[SVC-002]** إدارة المستندات (Document Management) ✅ (باكند + فرونتإند)
  - [x] جدول `documents` + `document_versions`
  - [x] رفع وتخزين المستندات + ترقيم تلقائي (DOC-YYYY-XXXX)
  - [x] إدارة الإصدارات (version history) + رفع إصدار جديد
  - [x] البحث في المستندات (عنوان + وصف + وسوم)
  - [x] وصول آمن حسب الصلاحيات (عام / شركة / قسم / خاص)
  - [x] تصنيفات: عام، عقد، فاتورة، تقرير، سياسة، دليل
...
- [x] **[TAX-002]** إشعارات مواعيد التقديم الضريبي ✅ (باكند + فرونتإند)
  - [x] تقويم المواعيد الضريبية ✅ — `GET /taxes/calendar` + `pages/Taxes/TaxCalendar.jsx`
  - [x] إشعارات قبل الموعد (7/3/1 أيام) ✅ — `reminder_days` field في `tax_calendar`
...
| ✅ المرحلة 8: إكمال الوحدات للوصول إلى 100% | 65+ | ✅ مكتمل (POS-001/002/005 ✅ + SALES-005 ✅ + DirectLaborReport ✅) |
| 🔵 المرحلة 9: التكامل الخارجي | 50+ | 🟡 جارٍ (API Keys ✅ + Webhooks ✅ + CRM-002 ✅ + CRM-004 ✅ + SVC-001 ✅ + SVC-002 ✅) |
...
| الفئة | العدد |
|-------|-------|
| إجمالي API Endpoints | ~390+ |
| إجمالي صفحات Frontend | ~166+ |
| إجمالي أسطر Backend | ~27,000+ |
| جداول قاعدة البيانات | 126+ |
| Triggers | 25 |
| Indexes | 40+ |
...
**الحالة:** المرحلة 8 مكتملة ✅ — المرحلة 9 جارية (API Keys + Webhooks + CRM-002/004 + SVC-001/002) — المرحلة 10 بدأت (ZATCA-002/004 + WHT)  
**التقييم:** تم إنجاز حوالي 95% من النظام الأساسي — الجديد: إدارة الخدمات والصيانة + إدارة المستندات (باكند + فرونتإند)
